...
- **`src/lib/<modulo>/`** – por módulo (comercial, educacional, estoque, etc.):
...
- **`src/lib/` ou `src/lib/`** – entidades e tipos compartilhados (ex.: Pessoa, membro) quando usados por mais de um módulo.
- **`src/lib/`** – cliente Supabase, criação de repositórios, config; sem regras de negócio.

Nomenclatura de arquivos: `PessoaService.ts`, `MemberRepository.ts`, `Pessoa.ts` (entidade). Um service ou repository por arquivo.

---

## Convenções de nomenclatura

- **Services:** sufixo `Service` (ex.: `DeliveryService`, `ContratoService`). Métodos que exprimem ação ou caso de uso (ex.: `concluirEntrega`, `buscarPorId`).
- **Repositories:** sufixo `Repository` (ex.: `MemberRepository`, `DeliveryRepository`). Métodos que exprimem acesso a dados (ex.: `findById`, `insert`, `update`).
- **Entidades:** nome do domínio (ex.: `Pessoa`, `membro`, `Contrato`). Podem ter métodos de comportamento quando fizer sentido (ex.: `membro.estaAtivo()`).
...
- O **cliente Supabase** é instanciado em `src/lib/` (ou recebido por factory). Repositories recebem o cliente (ou uma interface) no construtor.
...
- Exemplo de assinatura: `constructor(private readonly pessoaRepository: MemberRepository)` (ou equivalente em TypeScript).
...
|--------|--------|------------------------|
| **DTOs de entrada/saída** | Definido | Sufixo **Dto**. Local: `src/lib/<modulo>/dtos/`. Ex.: `CriarEntregaDto`. Dados que entram no service (entrada de API ou de outro módulo) devem ser tipados com DTOs. |
...
O chatbot usa tools em `src/lib/deliveries.ts` para consultar dados. A tool `auditar_campo` é **genérica**: aceita qualquer campo do catálogo `CAMPOS_AUDITAVEIS`.
...
- **Nomenclatura:** sufixo `Dto` (ex.: `CriarEntregaDto`, `CriarContratoDto`).
- **Local:** `src/lib/<modulo>/dtos/`.
...
Usar uma classe base para erros da camada de aplicação, com código e status HTTP, em vez de strings ou `Error` genérico. Local sugerido: `src/lib/types.ts` (ou `src/lib/`).
...
- **Pessoa** (cliente/membro): uma única entidade; referenciar por ID em todos os módulos.
- **Usuário** (Auth + perfil): `user_id` para operações vinculadas ao logado.
- Services e repositories que tocam em pessoa ou membro devem usar sempre o mesmo identificador (ex.: `pessoa_id`, `member_id`) e não criar cadastros duplicados. Alinhar à skill Entidades centrais quando existir.
...
- [KPI F3F Supabase / Engenheiro de dados](.claude/skills/kpi-supabase-data-engineer/SKILL.md) – esquema, RLS, tipos gerados.
- data-flow.md – fluxo de dados e entidades compartilhadas.
