...
- **Ainda não temos** biblioteca ou componente para cada tipo (ex.: calendário). Na medida em que o projeto adotar uma lib ou criar um componente, **atualizar esta skill** (e o [reference.md](reference.md)) com:
  ...
  - Caminho do componente no repositório (ex.: `src/shared/ui/InputCpf.tsx`).
...
- **Listas e filtros:** definir ou alterar DataTable, FiltroBuscaTexto, DateRangePicker, BarraFiltrosPadrao, coluna-manager ou useColunasPersistidas; registrar no reference § "Listas e filtros padronizados". Requisito: [requisitos-listas-e-filtros-padronizados.md](.context/docs/requisitos/requisitos-listas-e-filtros-padronizados.md).
- **Mensagens e confirmação:** toasts, ConfirmDialog, chaves de mensagem em `src/shared/messages/`; requisito: requisitos-mensagens-padronizadas.md.
...
- **Localização:** componentes compartilhados em `src/shared/ui/`; importação via **barrel** `@/shared/ui` (ver [reference.md](reference.md) § Barrel). Manter consistência de nomes (ex.: `InputCpf`, `InputTelefone`, `InputData`, `InputMoeda`); ao criar componente novo, adicionar o export em `src/shared/ui/index.ts`.
...
- **Base de Componentes:** Use sempre os primitivos do **shadcn/ui** (localizados em `src/shared/ui/` ou `src/components/ui/`) como base para novos componentes padronizados.
...
Para gerar a estrutura inicial de um componente em `src/shared/ui/`, execute o script (a partir da raiz do repositório):

```bash
bash .cursor/skills/kpi-componentes/scripts/create-component.sh <Nome>
```

Exemplo: `bash .cursor/skills/kpi-componentes/scripts/create-component.sh InputData` gera `src/shared/ui/InputData.tsx` com interface `Props` e export em PascalCase. Em Windows use Git Bash ou WSL.
