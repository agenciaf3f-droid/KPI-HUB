...
Responsável por **traduzir** a linguagem "do mundo real" (rotinas do dia a dia, dores do usuário, regras de cobrança, processos) em **linguagem do sistema** (requisitos técnicos, campos necessários, fluxos de estado, integrações). É a **ponte** de engenharia de requisitos: deve ser a **primeira** a ser chamada pela **Skill Gerente** quando o usuário trouxer uma **ideia bruta de negócio** (ex.: "o aluno paga a mensalidade e libera o acesso"). O output alimenta Entidades centrais, Integrações, Backend, Supabase e UX; garante que o desenvolvedor (e a IA) entendam o **valor de negócio** por trás de cada botão e de cada regra.

## Regra de ouro

- **Input:** rotina do dia a dia, dor do usuário, regra de negócio ou cobrança descrita em linguagem de negócio (ex.: "quando o contrato é assinado, o aluno ganha acesso ao curso").
- **Output:** documento de requisitos técnicos (ou seção em doc existente) com: **o que** o sistema deve fazer; **quais campos/dados** são necessários; **fluxo de estados** (ex.: status do pedido, status do aluno, status do contrato); **eventos** (ex.: pagamento confirmado → liberar acesso); **critérios de aceite** (condições para o processo ser considerado concluído com sucesso, para a QA testar). Tudo alinhado a [project-plan.md](.context/docs/project-plan.md) e [data-flow.md](.context/docs/data-flow.md).
...
- **Valor de negócio explícito:** cada requisito ou fluxo deve deixar claro **por que** existe (ex.: "botão 'Liberar acesso' existe porque o financeiro confirma o pagamento e o aluno deve ver o conteúdo"; "campo status do aluno existe para controlar se pode acessar o portal").

## Quando usar esta skill

- **Ideia bruta de negócio:** "precisamos que o aluno pague a mensalidade e libere o acesso"; "quando o contrato for assinado, queremos disparar o onboarding"; "o mentor precisa ser notificado quando a tarefa está atrasada". A **Skill Gerente** deve acionar esta skill **primeiro** nesses casos.
...
- **Alinhar ao modelo existente:** usar entidades centrais (pessoa/aluno, user_id) e contratos entre módulos (skill Integrações) já definidos; não inventar entidades duplicadas. Se o requisito exigir nova entidade ou novo contrato, indicar e deixar para Entidades centrais ou Integrações definir.
...
- **Onde salvar:** convenção (ex.: `.context/docs/requisitos/` ou seção em project-plan); referência ao índice (Documentação).
...
- **Entidades centrais:** requisitos podem indicar necessidade de novo campo ou estado em pessoa/aluno; esta skill descreve o quê; Entidades define o modelo.
...
- Contexto de negócio e módulos: [project-plan.md](.context/docs/project-plan.md). Fluxo de dados: [data-flow.md](.context/docs/data-flow.md).
