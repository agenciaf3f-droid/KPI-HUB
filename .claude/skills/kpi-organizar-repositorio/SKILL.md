...
description: "Folder structure and repository organization for KPI F3F. Defines where to put files (src/app, src/lib/, src/shared, src/infra), cleans up src/ and root, bulk moves files and keeps directory conventions. Use when defining or changing folder structure, moving files, or fixing disorganization."
---

# KPI F3F Organizar repositório

Responsável pela **estrutura de pastas** e por **onde colocar arquivos** no KPI F3F. Define o mapa de diretórios (src/app, src/lib/, src/shared, src/infra, .context), desfaz bagunça em `src/` e na raiz, move arquivos em massa e mantém convenções. O **conteúdo** do código (limpeza, duplicação, refatoração) é da skill [Limpeza de código](.claude/skills/kpi-limpeza-codigo/SKILL.md); esta skill cuida da **organização física** (pastas e localização dos arquivos). Referência: [AGENTS.md](AGENTS.md) (Repository Map) e [project-plan.md](.context/docs/project-plan.md).
...
- **Limpeza de conteúdo** (código morto, duplicação, refatoração) → skill [Limpeza de código](.claude/skills/kpi-limpeza-codigo/SKILL.md); esta skill não altera o que está dentro dos arquivos, só onde eles ficam.

## Quando usar esta skill

- Definir ou **alterar a estrutura de pastas** do projeto (ex.: adicionar `src/lib/`, reorganizar módulos).
...
- **Criar um novo módulo** do ponto de vista de pastas (ex.: `src/lib/` com subpastas services, repositories, components) – a lógica e o código são das skills Backend/Frontend; esta skill define a **estrutura de diretórios** do módulo.
- **Mover arquivos em massa** (ex.: extrair todos os utils para `src/lib/utils.ts`) sem mudar o conteúdo; atualizar imports após a movimentação.
...
- **Mover sem quebrar:** ao mover arquivos, atualizar todos os imports; preferir **aliases** (`@/lib/*`, `@/lib/*`) em vez de caminhos relativos longos (ver [reference.md](reference.md) – Padrão de imports). Rodar `npm run build` (e testes) após reorganização.
...
- **Registro progressivo:** quando o projeto adotar uma convenção nova (ex.: pasta `src/lib/` para entidades compartilhadas), documentar no [reference.md](reference.md).

## Mapa de diretórios (resumo)

- **`src/app/`** – Next.js App Router (layouts, páginas, loading, error).
- **`src/lib/<modulo>/`** – por módulo (comercial, educacional, etc.): services, repositories, components, páginas do módulo quando aplicável.
- **`src/lib/`** – UI compartilhada, hooks, utils, tipos compartilhados entre módulos.
- **`src/lib/`** – cliente Supabase, auth, config de integrações.
...
- **Limpeza de código:** conteúdo (remover morto, unificar); esta skill = onde os arquivos ficam. Ao extrair código para shared, a Limpeza pode indicar "mover para src/lib/utils"; esta skill garante que a pasta existe e que o arquivo vai para o lugar certo.
