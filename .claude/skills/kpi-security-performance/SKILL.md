...
- **Auditar policies RLS** (um membro/usuário consegue ver ou alterar dados de outro?) → esta skill.
...
- **Implementar** RLS ou corrigir esquema → skill [Supabase / Engenheiro de dados](.claude/skills/kpi-supabase-data-engineer/SKILL.md); esta skill **aponta** o que está errado ou faltando.
...
- Pedidos como: *"Audite as policies RLS da tabela financeiro_transacoes. Um membro consegue ver dados de outro?"* → revisar políticas SELECT/INSERT/UPDATE/DELETE; verificar se há filtro por `member_id` ou `user_id` consistente com a regra de negócio; reportar se um usuário puder ver ou alterar dados de outro.
...
- **Pergunta central em RLS:** para cada tabela com dados por membro/usuário/tenant: "Um membro (ou usuário A) consegue ler ou modificar dados de outro membro (usuário B)?" Se sim, é falha; documentar e sugerir política correta (ex.: `USING (member_id = auth.uid()` ou via join com tabela de vínculo).
...
- **Referência de políticas:** usar security.md e o [reference da skill Supabase](.claude/skills/kpi-supabase-data-engineer/reference.md) para saber o padrão esperado de RLS; a auditoria verifica conformidade.

## Tipos de tarefa (resumo)

| Pedido / contexto | Ação |
|-------------------|------|
| Auditar RLS da tabela X. membro vê dados de outro? | Revisar políticas da tabela; verificar USING/WITH CHECK por member_id/user_id/auth.uid(); reportar se possível ver/alterar dados de outro. |
...
- Políticas de segurança do projeto: security.md.
