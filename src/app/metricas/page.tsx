...
import { loadActiveTimers } from "@/lib/deliveries";
import { loadGamification, type GamificationData } from "@/lib/gamification";
...
export default async function MetricsPage() {
  ...
  const activeTimers = await loadActiveTimers(profile.organization_id);
  const gamification = await loadGamification(profile);

  return <MetricsDashboard fullName={profile.full_name} role={profile.role} activeTimers={profile.role === "admin" ? activeTimers : activeTimers.filter((timer) => timer.assigneeName === profile.full_name)} gamification={gamification} realtimeTopic={`creator-monitor:${profile.organization_id}`} />;
}

export function MetricsDashboard({ fullName, role, queueHref, metricsHref, accountHref, showQueue = true, activeTimers = [], gamification, realtimeTopic, adminAccessToken }: { fullName: string; role: "admin" | "designer"; queueHref?: string; metricsHref?: string; accountHref?: string | null; showQueue?: boolean; activeTimers?: ActiveDeliveryTimer[]; gamification?: GamificationData; realtimeTopic?: string; adminAccessToken?: string }) {
  const isAdmin = role === "admin";

  return (
    <div className="min-h-svh bg-background text-foreground md:pl-28">
      <AppHeader activeItem="metricas" fullName={fullName} queueHref={queueHref} metricsHref={metricsHref} accountHref={accountHref} showQueue={showQueue} />

      <main className="mx-auto w-full max-w-[1480px] p-4 sm:p-6 lg:p-8">
        {adminAccessToken ? <div className="mb-4 flex justify-end"><AdminCreateTaskButton adminAccessToken={adminAccessToken} /></div> : null}
